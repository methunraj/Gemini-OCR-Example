...
-----------------------------------
Contains system and user prompts for extracting data from OCR text of historical Russian documents.
"""

# System prompt for text processing
SYSTEM_PROMPT = """You are a specialized extraction assistant designed for comprehensive JSON data processing from historical Russian documents. Your primary function is to extract ALL entries from the provided text and convert them into properly structured JSON data.
...
You must:
1. Process EVERY SINGLE entry visible in the provided text.
...
# User prompt template for text processing
USER_PROMPT_TEMPLATE = """Extract all the information from the provided historical Russian document text and convert it into a JSON array following the exact schema provided below. You MUST process and include EVERY entry from the text.
...
For each entry in the provided text, create a separate object in the JSON array following the EXACT format shown above. DO NOT combine multiple entries into a single object.
...
def get_text_prompt_parts(schema, examples=None):
    """
    Generate prompt parts for text processing.
