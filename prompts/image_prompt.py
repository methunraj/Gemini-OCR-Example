...
# Critical field handling instructions
CRITICAL_FIELD_INSTRUCTIONS = """
CRITICAL PROVINCE FIELD INSTRUCTION: When extracting the value for the 'Province' field, ensure that you capture the abbreviation 'губ.' if it is present in the corresponding cell in the image. Do not omit this abbreviation. For example, if the image shows 'Смоленской губ.', the JSON 'Province' field should also be 'Смоленской губ.'.

CRITICAL SHORT FORM PRESERVATION INSTRUCTION: Extract all text exactly as it appears in the image. Do not expand any abbreviations or short forms (For example, if the image contains "Рядов.", the JSON should also contain "Рядов.", not "Рядовой" or any other full form).

CRITICAL TOWN AND PLACE FIELD HANDLING:
- Look for the column with the header "Какого уѣзда." And a cell containing the keyword "гор." This cell represents the 'Town' field in the JSON. Extract the values from this cell and map them to the 'Town' field.
- Map the values in the column with the header 'Какой волости, села, деревни или станицы.' (Which Volost, Selo, Derevnya or Stanitsa) to the 'Place' field in the JSON.

CRITICAL CHARACTER HANDLING:
- The alpha characters should all be from the pre-1918 Russian Cyrillic alphabet. There will be no Latin alphabet characters, even when they look very similar e.g. a Latin K looks very similar to a Cyrillic К but is coded differently, and the OCR must output only Cyrillic characters.
"""
...
def get_image_prompt_parts(schema, examples=None):
    ...
    # Add the critical field instructions
    prompt_parts[1] += CRITICAL_FIELD_INSTRUCTIONS
